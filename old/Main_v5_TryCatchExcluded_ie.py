...
def click_on_view_orders():
    ...
    button = driver.find_element_by_xpath(xpathsearch)
    button.click()
...
def click_all_buttons_overview(xpathsearch_firstall,xpathsearch_secondall):
    ...
    driver.find_element_by_xpath(xpathsearch_secondall).click()
    print_with_timestamp('Focus on the second "all" button and click it')
    driver.find_element_by_xpath(xpathsearch_firstall).click()
    print_with_timestamp(' ')
    time.sleep(15*delay_factor)
    if driver.find_element_by_id('ctl00_right_side_openInNewWindow').is_selected() == False:
        driver.find_element_by_id("ctl00_right_side_openInNewWindow").click()
...
def check_exists_by_type(elementtype, element_id_or_path, ticker):
    ...
    while result == False and x <= ticker:
        ...
        try:
            time.sleep(1)
            if elementtype == 'id':
                driver.find_element_by_id(element_id_or_path)
            elif elementtype == 'xpath':
                driver.find_element_by_xpath(element_id_or_path)     
            result = True
        except:
            result = False
    return result
...
def check_exists_by_value(elementtype, element_id_or_path, ticker, textvalue_to_find):
    ...
    while result == False and x <= ticker:
        ...
        try:
            time.sleep(1)
            if elementtype == 'id':
                textvalue_found = driver.find_element_by_id(element_id_or_path).text
            elif elementtype == 'xpath':
                textvalue_found = driver.find_element_by_xpath(element_id_or_path).text    
...
def get_current_status():
    ...
    current_status = driver.find_element_by_xpath(xpathsearch_statusbar).text
...
def get_current_status_line(page):
    ...
    current_status = driver.find_element_by_xpath(xpathsearch_statusline).text
    if "Cancelled" not in current_status: 
        current_status_index = statusFlow.index(current_status)
        print_with_timestamp('   The current status is: ' + current_status + ' (' + str(current_status_index) + ')')
    else:
...
def get_production_substatus():
    ...
    current_status = driver.find_element_by_xpath(xpathsearch_streamics_status).text
...
label_spacer2.grid(column=0, row=0)
...
label_combo.grid(column=0, row=4)
...
label_spacer.grid(column=0, row=6)

Livit = IntVar()
Checkbutton(window, text="Livit cases?", variable=Livit).grid(column=0, row=7)

# label_spacer3 = Label(window, text=" ")
# label_spacer3.grid(column=0, row=8)

# label_combo_slower = Label(window,text="Increase waiting time:")
# label_combo_slower.grid(column=0, row=9)
# combo_slower = Combobox(window,width=40)
# combo_slower['values']= delayFactor
# combo_slower.current(0) #set the selected item
# combo_slower.grid(column=0, row=10)

label_spacer4 = Label(window, text=" ")
label_spacer4.grid(column=0, row=11)

label_email = Label(window, text="Email address")
label_email.grid(column=0, row=12)

# entry_email = Entry(window, text="")
# entry_email.grid(column=0, row=13)

entry_email = Combobox(window,width=width_inputfield)
entry_email['values']= ("Choose an email address", "rkia.elhassani@rsprint.be","julie.wellens@materialise.be", "sander.van.nieuwenhoven@rsprint.be", "pieter-jan.lijnen@rsprint.be", "lode.bosmans@rsprint.be","flowbuiltproduction@gmail.com")
entry_email.current(0) #set the selected item
entry_email.grid(column=0, row=13)

label_spacer5 = Label(window, text=" ")
label_spacer5.grid(column=0, row=14)

label_password = Label(window,text="Password")
label_password.grid(column=0, row=15)

entry_password = Entry(window,show="*",width=width_inputfield,text="")
entry_password.grid(column=0, row=16)

label_spacer6 = Label(window, text=" ")
label_spacer6.grid(column=0, row=17)
...
if len(caseids) > 0:
    ...
    # driver.set_page_load_timeout(300*delay_factor*delay_factor)
    print_with_timestamp(' ')
    print_with_timestamp('Opening the website')

    # Insert credentials if available
    if email != "":
        driver.find_element_by_id("ctl00_left_side_ucL_loginView_accP_vL_cL_UserName").clear()
        # Grab the login field
        searchinput = driver.find_element_by_id("ctl00_left_side_ucL_loginView_accP_vL_cL_UserName")
        # Insert the email address
        searchinput.send_keys(email)
    if password != "":
        driver.find_element_by_id("ctl00_left_side_ucL_loginView_accP_vL_cL_Password").clear()
        # Grab the password field
        searchinput = driver.find_element_by_id("ctl00_left_side_ucL_loginView_accP_vL_cL_Password")
        # Insert the password 
        searchinput.send_keys(password)
    if email != "" and password != "":
        # Press the button
        button = driver.find_element_by_id("ctl00_left_side_ucL_loginView_accP_vL_cL_LoginButton")
        button.click()
    ...
    current_role = driver.find_element_by_id(xpathsearch_OmsRole).text
    ...
    time.sleep(1*delay_factor)
    driver.find_element_by_xpath(xpathsearch_batchall).click()
    ...
    for caseid in caseids:
        ...
        check_first_all = driver.find_element_by_xpath(xpathsearch_firstall_pressed).get_attribute('aria-pressed')
        check_second_all = driver.find_element_by_xpath(xpathsearch_secondall_pressed).get_attribute('aria-pressed')
        ...
        driver.find_element_by_id("gs_CaseID").clear()
        # Grab the case ID search field
        searchinput = driver.find_element_by_id("gs_CaseID")
        ...
        caseid_nr = driver.find_element_by_xpath(xpathsearch_id).get_attribute("id")
        ...
        if shouldBeLivit:
            ...
            if driver.find_element_by_xpath(xpathsearch_companyname).text == 'Livit Orthopedie bv Company':
                isIndeedLivit = True
            else:
                isIndeedLivit = False
                # Show error message
            ...
            if (shouldBeLivit == True and isIndeedLivit == True) or (shouldBeLivit == False and isIndeedLivit == False):
                ...
                while current_status_index < destination_status_index and no_error and new_loop:
                    if statusFlow[current_status_index] == 'Production' and first_production_run:
                        ...
                        driver.find_element_by_xpath(xpathsearch).click()
                        ...
                        if take_order_button_present:
                            ...
                            button = driver.find_element_by_id(take_order_button_id)
                        ...
                        while element_status2 == False:
                            ...
                            time.sleep(0.1)
                            button = driver.find_element_by_xpath(xpathsearch_partslist_button)
                            time.sleep(1*delay_factor)
                            button.click()
                            time.sleep(0.1)

                            classes = driver.find_element_by_xpath(xpath_partslist_active).get_attribute('class')
                        ...
                        if current_production_substatus != 'Streamics (Built)':
                            ...
                            button = driver.find_element_by_xpath(xpathsearch_update_allbutton)
                            time.sleep(1*delay_factor)
                            button.click()
                        ...
                        button = driver.find_element_by_xpath(xpathsearch_promotionbutton)
                        time.sleep(3*delay_factor)
                        button.click()
                        time.sleep(0.1*delay_factor)
                        ...
                        time.sleep(3*delay_factor)
                        button = driver.find_element_by_xpath(xpathsearch_confirmbutton)
                        time.sleep(0.1*delay_factor)
                        button.click()
                        ...
                        error_button_present = driver.find_element_by_xpath(xpathsearch_error_button).value_of_css_property("display") 
                        ...
                        if error_button_present != "none":
                            ...
                            driver.find_element_by_xpath(xpathsearch_error_button_specific).click()
                            time.sleep(1*delay_factor) # 5
                    ...
                    if (current_status_index >= statusFlow.index('Production')) and (current_status_index < destination_status_index ):
                        ...
                        try:
                            ...
                            driver.find_element_by_xpath(promotion_button_confirm3).click()
                        except:
                            DoNothing = 1
                        ...
                        time.sleep(0.1)
                        driver.find_element_by_id(promotion_button_confirm).click()
                        time.sleep(1*delay_factor)
                        ...
                        time.sleep(0.1)
                        driver.find_element_by_xpath(promotion_button_confirm2).click()
                        time.sleep(5*delay_factor)
                        # Confirm the promotion again
                        xpath_conformation_text = '/html/body/form/div[11]/span/div/div'
                        confirmation_text = driver.find_element_by_xpath(xpath_conformation_text).text
                        ...
                        # 
                        try:
                            driver.find_element_by_id(checkbox_ID)
                        except:
                            new_loop = False
    ...
    print_with_timestamp(' ')
    print_with_timestamp('Valid promotions (' + str(len(caseids_valid)) + '/' + str(len(caseids)) + ')')
    print_with_timestamp('----------------------------')
    print_casebycase(caseids_valid)
    print_with_timestamp(' ')
    print_with_timestamp('Invalid promotions (' + str(len(caseids_invalid)) + '/' + str(len(caseids)) + ')')
    print_with_timestamp('----------------------------')
